...
// GET /api/games/:userId/:gameId
async function loadGame(req, res, next) {
  try {
    const { gameId } = req.params;
    const [tab, cof, ben, aco, jog] = await Promise.all([
      pool.query('SELECT * FROM economia_tabuleiro  WHERE game_id = $1', [gameId]),
      ...
      pool.query('SELECT nr_jogador, no_jogador, vl_dinheiro, vl_deve, nr_posicao, ao_presente FROM economia_jogadores WHERE game_id = $1 ORDER BY nr_jogador', [gameId]),
    ]);
...
async function saveGame(req, res, next) {
  ...
  try {
    ...
    const { userId = 1, gameId, tabuleiro: t, cofrinhos, bens, acoes, jogadores } = req.body;
    ...
    if (jogadores && jogadores.length > 0) {
      const placeholders = jogadores.map((_, i) =>
        `($1, $2, $${i * 6 + 3}, $${i * 6 + 4}, $${i * 6 + 5}, $${i * 6 + 6}, $${i * 6 + 7}, $${i * 6 + 8})`
      ).join(', ');
      const params = [newGameId, userId];
      jogadores.forEach(j => params.push(
        j.nrJogador, j.noJogador, j.vlDinheiro, j.vlDeve, j.nrPosicao, j.aoPresente
      ));
      await client.query(
        `INSERT INTO economia_jogadores(game_id,user_id,nr_jogador,no_jogador,vl_dinheiro,vl_deve,nr_posicao,ao_presente)
         VALUES ${placeholders}`,
        params
      );
    }
...
// DELETE /api/games/:userId/:gameId
async function deleteGame(req, res, next) {
  try {
    const { gameId } = req.params;
    await pool.query('DELETE FROM economia_tabuleiro WHERE game_id = $1', [gameId]);
    res.json({ ok: true });
  } catch (err) {
    next(err);
  }
...
// PATCH /api/games/:userId/:gameId/name
async function renameGame(req, res, next) {
  try {
    const { gameId } = req.params;
    const { name } = req.body;
    await pool.query(
      'UPDATE economia_tabuleiro SET no_estado = $1 WHERE game_id = $2',
      [name, gameId]
    );
    res.json({ ok: true });
  } catch (err) {
